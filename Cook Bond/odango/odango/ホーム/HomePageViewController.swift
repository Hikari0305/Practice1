...
//        seasonalRecipeScrollView.flashScrollIndicators()
    ...
    func createContentsView() -> UIView {
        ...
        let seasonalScrollViewy = seasonalRecipey + margin + 30
        ...
        seasonalView.frame = CGRect(x: 0, y: 0, width: 1500, height: seasonalView_height)
        seasonalScrollView.addSubview(seasonalView)
        
        let seasonalImage = UIButton()
        seasonalImage.frame = CGRect(x: 0, y: 0, width: 170, height: 170)
        seasonalImage.setBackgroundImage(UIImage(named: "unnamed.jpg"), for: .normal)
        setUp(seasonalImage, UIImage(named: "unnamed.jpg")!)
        seasonalView.addSubview(seasonalImage)
        
        let label = UILabel()
        let label_x = seasonalView.center.x
        let label_y = seasonalView.center.y
        label.frame = CGRect(x: 0, y: 170, width: 170, height: 50)
        label.text = "odango"
        ...
        let preferencesy = seasonalRecipey + seasonalView_height + margin*2
